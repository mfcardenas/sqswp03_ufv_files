...
function showLoadingState(mode) {
    ...
    const bodyText =
        mode === 'ollama'
            ? 'Talking to the local Ollama instance. This may take a few seconds while the model drafts new ISO 29148 labs.'
            : 'Retrieving the curated requirement studio pack from the JSON bundle.';
    placeholder.append(
        loader,
...
function showErrorState(message) {
    ...
    const friendlyMessage = isOllamaError
        ...
        : 'Unable to build the requirement scenario.';
    panel.append(
...
function showDefaultPlaceholder() {
    ...
    placeholder.append(
        ...
        createElement('p', {
            text: 'Pick the content source and load the scenario to begin the ISO 29148 requirement studio.'
        })
    );
    appRoot.appendChild(placeholder);
}
