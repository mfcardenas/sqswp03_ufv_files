(() => {
    ...
    const dom = {
        ...
        predefinedLabel: document.getElementById("predefined-label"),
    ...
    const renderRanking = (container, ranking) => {
        ...
        const totalQ = state.game ? state.game.questions.length : "?";
        ranking.slice(0, 10).forEach((entry, idx) => {
            const avg = entry.avgTime ? `${entry.avgTime.toFixed(1)}s` : "--";
            const correct = typeof entry.correctCount === "number" ? entry.correctCount : "?";
            list.insertAdjacentHTML("beforeend",
                `<li>${idx + 1}. ${entry.nickname} · ${entry.score} pts · ✅ ${correct}/${totalQ} · ⏱️ ${avg}</li>`);
        });
        container.innerHTML = "";
    ...
    const createSession = (config) => {
        ...
        socket.emit("host:createSession", config, (res) => {
            ...
            dom.sessionCode.textContent = `${uiLang === "en" ? "Code" : "Código"}: ${res.code}`;
            ...
            // ── Auto-attach game based on chosen source ──
            if (config.source === "llm") {
                attachLLMGame();
            } else {
                attachPredefinedGame();
            }
        });
    };
    ...
    dom.hostForm.querySelectorAll('input[name="source"]').forEach(radio => {
        radio.addEventListener('change', () => {
            // Toggle predefined selector visibility
            if (dom.predefinedLabel) {
                dom.predefinedLabel.style.display = radio.value === 'predefined' && radio.checked ? '' : 'none';
            }
        });
    });
