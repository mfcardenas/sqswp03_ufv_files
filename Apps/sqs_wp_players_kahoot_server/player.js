(() => {
    ...
    dom.playerForm.addEventListener("submit", (event) => {
        ...
        // ── Persistence: include userId + university from registration
        let userId = null, university = "";
        try {
            const stored = JSON.parse(localStorage.getItem("isoKahootPlayer"));
            if (stored) { userId = stored.userId; university = stored.university || ""; }
        } catch (_) { /* no stored player */ }
        socket.emit("player:joinSession", { nickname, code, userId, university }, (res) => {
            if (!res?.ok) {
